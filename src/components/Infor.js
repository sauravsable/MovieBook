    ...
    render() {
        return (
            <div className='container my-3'>
                <h1 style={{ textAlign: "center", fontSize: "3rem" }}>Movie-pedia</h1>
                <div className="row">
                    {this.state.samples.map((element) => {
                        return <div className="col-md-3" key={element.show.id}>
                            <InforItem name={element.show.name} summary={element.show.summary.replace(/<[^>]+>/g, '').slice(0, 150)} imageUrl={element.show.image.original} premiered={element.show.premiered} ended={element.show.ended ? element.show.ended : "Unknown"} language={element.show.language} showUrl={element.show.url} genre={element.show.genres.join(' ')} Rating={element.show.rating.average ? element.show.rating.average : "Unknown"} />
                        </div>
                    })}
